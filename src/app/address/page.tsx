...
export default function Address() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/address/createAdress', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`  // Se pasa el token en los encabezados
        },
        ...
        <form onSubmit={handleSubmit}>
          <div className="address-container">
            <div>
              ...
              <input
                placeholder="Ej. Avenida los leones"
                className="inputTextAddress"
                type="text"
                id="direccion"
                name="direccion"
                required
